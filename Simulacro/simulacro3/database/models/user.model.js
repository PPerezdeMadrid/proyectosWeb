...
users.register = function(username, password){
    ...
    users.generateHash(password, function(err, hash){
        ...
        users.data[username] = {username, hash, last_Login: new Date().toISOString, cookieAccepted: false};
    });
}
...
users.isCookiesAccepted = async function(username){
    console.log("Accediendo a la base de datos para comprobar si las cookies están aceptadas...");
